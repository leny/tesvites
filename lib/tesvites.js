 ...
 */
"use strict";
var Duration, chalk, dStart, error, moment, options, pkg, program, speedtest;
...
(program = require("commander")).version(pkg.version).usage("[options]").description(pkg.description).option("-m, --max-time <max-time>", "The maximum length of a single test run (upload or download)").option("-p, --ping-count <ping-count>", "The number of close servers to ping to find thew fastest one").option("-s, --servers <servers>", "The number of servers to run a download test on. The fastest is used for the upload test and the fastest result is reported at the end.").option("-i, --id <server-id>", "ID of the server to restrict the tests against.").option("-l, --log-output", "Show the complete output of the test.").parse(process.argv);
...
speedtest.visual(options, function(error, results) {
  var oDuration;
